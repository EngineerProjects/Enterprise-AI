...
> **English**: an English version of this guide is available in [CONTRIBUTING.md](CONTRIBUTING.md).
...
```bash
export ANTHROPIC_API_KEY="sk-ant-..."   # pour les tests d'intégration (optionnel)
```
...
# Itère sur les messages et ajoute chacun en mémoire
for msg in messages:
...
- stdlib, puis tiers, puis projet — séparés par une ligne vide
...
- Mocker les appels API LLM avec un `Provider` fake (voir exemples dans `tests/`)
...
Uniquement les paquets nécessaires à **chaque session agent**. Épinglées à la version exacte (`==X.Y.Z`).
...
WIP
update code
feat: Ajouter une super nouvelle feature qui fait plein de choses et corrige aussi des bugs
```
