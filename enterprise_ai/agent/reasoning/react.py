...
class ReActReasoning(ToolBasedReasoning):
                ...
                for tool_call in tool_calls:
                    ...
                    try:
                        ...
                        if tool_result.error:
                            observation = f"Observation: Error executing {tool_name}: {tool_result.error}"
                        else:
                        ...
                        updated_messages.append(cast(MessageProtocol, Message.user_message(observation)))
    ...
    async def execute_tools_parallel(
        ...
        **kwargs: Any
        ...
        try:
            ...
            # Process results
            return [(name, self._process_parallel_result(name, result)) 
                   for name, result in zip(tool_names, results)]
        except Exception as e:
            logger.error(f"Error executing tools in parallel: {e}", exc_info=True)
            # Return error results
            return [(name, ToolFailure(
    ...
    def _is_react_format(self, content: str) -> bool:
        ...
        react_patterns = [
            r"Thought:", r"Action:", r"Observation:", r"Answer:", r"I need to", r"First,"
        ]
    ...
    def process_task(self, agent: AgentProtocol, task: Task, **kwargs: Any) -> TaskStatus:
        ...
        system_prompt = self.format_system_prompt(agent, "", tools_description=tools_description, **kwargs)
                ...
                if not tool_calls:
                    ...
                    if action_info:
                        ...
                        if action_counter[action_key] > 2 and not backtracking:
                            ...
                            message_list.append(Message.user_message(
                                "I notice you're repeating the same action. Try a different approach or tool."
                            ))
                            continue
                        
                        # Execute tool
                        tool_result = self._execute_tool_with_handling(agent, tool_name, params, **kwargs)
    ...
    def _is_task_completed(self, content: str) -> bool:
        ...
        completion_patterns = [
            r"Answer:", r"Final Answer:", r"Task completed",
            r"I've completed the task", r"The answer is", r"Here is the answer"
        ]
    ...
    def _extract_action_from_text(self, content: str) -> Optional[Tuple[str, Dict[str, Any]]]:
        ...
        looser_action_match = re.search(r"(?:use|call|execute)\s+the\s+(\w+)\s+tool", content, re.IGNORECASE)
