...
class ReasoningManager:
    ...
    def select_framework_for_input(self, input_text: str) -> str:
        ...
        if any(
            kw in input_text.lower() for kw in ["code", "function", "class", "program", "debug"]
        ):
            return "swe"

        # Check for tool-use indications
        if any(
            kw in input_text.lower() for kw in ["use tool", "search", "look up", "tool", "find"]
        ):
            return "react"
    ...
    async def process_input(
        ...
        try:
            ...
            logger.error(
                f"Error processing input with framework {self.current_framework_name}: {e}"
            )
            ...
            if self.current_framework_name != self.config.default_framework:
                ...
                try:
                    ...
                    return await self._process_task_with_framework(
                        self._get_framework(self.config.default_framework), task, **kwargs
                    )
                except Exception as fallback_error:
    ...
    async def _process_task_with_framework(self, framework: Any, task: Any, **kwargs: Any) -> Any:
