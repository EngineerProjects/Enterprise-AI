...
class ExecutionManager:
    ...
    async def execute_with_timeout(
        self, context: ExecutionContext, func: Any, *args: Any, **kwargs: Any
    ) -> Any:
        ...
        if context.timeout:
            try:
                ...
                # Log timeout
                logger.warning(
                    f"Execution {context.execution_id} timed out after {context.timeout} seconds"
                )
    ...
    async def execute_in_parallel(
        ...
        # Create execution context
        context = self.create_execution_context(
            ExecutionType.BATCH, metadata={"function_count": len(functions)}, timeout=timeout
        )
        execution_id = context.execution_id
