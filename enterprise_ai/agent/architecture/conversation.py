...
class ConversationManager:
    ...
    def __init__(self, agent: Any, config: Optional[ConversationManagerConfig] = None):
        ...
        self._conversation_metadata: Dict[str, Dict[str, Any]] = {"default": {
    ...
    def set_system_message(self, message: Union[str, MessageProtocol]) -> None:
        ...
        if isinstance(message, str):
            ...
            if message.role != "system":
                logger.warning(f"Non-system message role provided: {message.role}, converting to system")
    ...
    def add_message(
        ...
        if (
            (processed_message.role == "system" and not self.config.include_system_messages) or
            (processed_message.role == "tool" and not self.config.include_tool_messages) or
            (processed_message.role in self.config.message_filter)
        ):
    ...
    def get_conversation_summary(self, conversation_id: str = "default") -> str:
        ...
        duration = (
            datetime.now() - meta["created_at"]
        ).total_seconds() if meta["created_at"] else 0
        ...
        for i in range(min(3, len(user_messages))):
            if i < len(user_messages):
                ...
                for j, msg in enumerate(reversed(assistant_messages)):
                    if j >= i and msg.timestamp and user_msg.timestamp and msg.timestamp > user_msg.timestamp:
    ...
    def _extract_topics(self, text: str) -> List[str]:
        ...
        stop_words = {
            ...
            "for", "with", "about", "as", "from", "is", "are", "am", "was", "were",
