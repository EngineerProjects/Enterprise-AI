...
It only depends on core modules and doesn't create circular imports.
"""
...
from enterprise_ai.exceptions import LLMError, TokenLimitExceeded, ContextWindowExceededError
from enterprise_ai.constants import DEFAULT_MAX_TOKENS, DEFAULT_TEMPERATURE
...
class LLMProvider(ABC, ProviderProtocol):
    """Abstract base class for LLM providers.

    This class defines the interface that all LLM providers must implement,
    providing a consistent API for different LLM services like OpenAI, Anthropic,
    and Ollama.
    """
    ...
    ) -> Generator[MessageProtocol, None, None]:
    ...
    async def acomplete_stream(
        self, messages: List[MessageProtocol], **kwargs: Any
    ) -> AsyncGenerator[MessageProtocol, None]:
...
class ConversationManager(ConversationManagerProtocol):
    ...
    This class handles message history, token counting, and context window
    management for conversations with LLMs.
    """
