...
class Bash(BaseTool):
    ...
    * Returns: Command output with session PID, execution status, and session management information
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        """Execute bash command with enhanced session management and smart execution tracking."""
    ...
    def _analyze_command_for_logging(self, command: str) -> Dict[str, Any]:
        """
        Analyze bash command to determine what kind of work it's doing.
        
        This helps determine if the execution is worth logging as a source.
        """
        command_lower = command.lower().strip()
        ...
        analysis = {
            ...
            'is_package_management': False
        }
        
        # Check for data processing commands
        data_commands = ['grep', 'awk', 'sed', 'sort', 'uniq', 'cut', 'tr', 'jq', 'csvtool']
        if any(cmd in command_lower for cmd in data_commands):
            analysis['is_data_processing'] = True
            analysis['command_type'] = 'data_processing'
        
        # Check for file operations
        file_commands = ['ls', 'find', 'cat', 'head', 'tail', 'cp', 'mv', 'rm', 'mkdir', 'touch']
        if any(cmd in command_lower for cmd in file_commands):
            analysis['is_file_operation'] = True
            if analysis['command_type'] == 'general':
                analysis['command_type'] = 'file_operations'
        
        # Check for system information commands
        system_commands = ['ps', 'top', 'htop', 'df', 'du', 'free', 'uname', 'lscpu', 'lsblk', 'systemctl']
        if any(cmd in command_lower for cmd in system_commands):
            analysis['is_system_info'] = True
            analysis['command_type'] = 'system_info'
        
        # Check for network operations
        network_commands = ['curl', 'wget', 'ping', 'netstat', 'ss', 'nslookup', 'dig']
        if any(cmd in command_lower for cmd in network_commands):
            analysis['is_network_operation'] = True
            analysis['command_type'] = 'network'
        
        # Check for package management
        package_commands = ['apt', 'yum', 'dnf', 'pip', 'npm', 'cargo', 'conda']
        if any(cmd in command_lower for cmd in package_commands):
            analysis['is_package_management'] = True
            analysis['command_type'] = 'package_management'
