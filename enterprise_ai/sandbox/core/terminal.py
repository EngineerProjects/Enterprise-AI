...
logger = get_logger("sandbox.terminal")
        ...
        try:
            if self.socket:
                ...
                try:
                    ...
                    logger.debug(f"Ignoring error while sending exit command: {e}")
                ...
                try:
                    ...
                    logger.debug(f"Socket shutdown error (expected in some cases): {e}")
            ...
            if self.exec_id:
                try:
                    ...
                    logger.debug(f"Error inspecting exec instance: {e}")
                ...
                while True:
                    try:
                        if self.socket:  # Add null check
                            ...
                            raise RuntimeError("Socket disconnected during operation")
...
class AsyncDockerizedTerminal:
    def __init__(
        ...
        self.container = (
            container if isinstance(container, Container) else self.client.containers.get(container)
        )
    ...
    async def init(self) -> None:
        ...
        if self.session:  # Add null check
