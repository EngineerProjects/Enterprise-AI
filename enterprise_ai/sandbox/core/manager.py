...
Manages multiple DockerSandbox instances, handling their lifecycle
...
from typing import AsyncGenerator, Dict, Optional, Set, Any, Type
    ...
    async def create_sandbox(
        ...
        async with self._global_lock:
            if len(self._sandboxes) >= self.max_sandboxes:
                logger.error(f"Maximum number of sandboxes ({self.max_sandboxes}) reached")
            ...
            if not await self.ensure_image(config.image):
                logger.error(f"Failed to ensure Docker image: {config.image}")
            ...
            try:
                ...
                logger.info(f"Created sandbox with ID: {sandbox_id}")
                return sandbox_id
