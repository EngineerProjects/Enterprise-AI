...
class ReActReasoning(ToolBasedReasoning):
    ...
    def _process_previous_tool_calls(
        ...
        if last_assistant_msg:
            # Extract provider information from LLM if available
            provider_name = None
            llm_provider = kwargs.get("llm_provider")
            if llm_provider and hasattr(llm_provider, "name"):
                provider_name = llm_provider.name
            
            # Parse tool calls using the enhanced parser with provider context
            tool_calls = parse_message_for_tool_calls(last_assistant_msg, provider_name=provider_name)
            ...
            if tool_calls:
                ...
                for tool_call in tool_calls:
                    ...
                    try:
                        ...
                        # Format result using provider-specific format if possible
                        tool_response = format_tool_response_message(
                            tool_name, 
                            tool_result, 
                            tool_call_id=tool_call_id,
                            response_format="provider" if provider_name else "react",
    ...
    def _extract_action_from_text(self, content: str) -> Optional[Tuple[str, Dict[str, Any]]]:
        """
        Extract tool action from text content with enhanced pattern recognition.
        ...
        """
        # Try to use the LLM adapter if available
        try:
            from enterprise_ai.llm.adapters import ToolAdapter
            adapter = ToolAdapter()
            
            # Create a basic message with the content
            from enterprise_ai.schema import Message
            msg = Message(role="assistant", content=content)
            
            # Extract tool calls using the adapter
            tool_calls = adapter.extract_tool_calls(msg)
            ...
            logger.debug(f"Error using adapter to extract tool calls: {e}")
        ...
        # Check for Anthropic's XML tool call format
        tool_xml_matches = re.search(r"<tool_call>(.*?)</tool_call>", content, re.DOTALL)
        if tool_xml_matches:
            tool_text = tool_xml_matches.group(1)
            name_match = re.search(r"<name>(.*?)</name>", tool_text) or re.search(r"<n>(.*?)</n>", tool_text)
            params_match = re.search(r"<parameters>(.*?)</parameters>", tool_text) or re.search(r"<p>(.*?)</p>", tool_text)
                    ...
                    try:
                        ...
                        for param_name, param_value in param_matches:
                            ...
                            if param_value.lower() in ["true", "false"]:
                                tool_params[param_name] = param_value.lower() == "true"
        ...
        # Check for antml function calls format
        antml_matches = re.search(r"<function_calls>(.*?)</function_calls>", content, re.DOTALL)
        if antml_matches:
            function_calls_text = antml_matches.group(1)
            invoke_match = re.search(r"<invoke name=\"([^\"]+)\">(.*?)</invoke>", function_calls_text, re.DOTALL)
            ...
            if invoke_match:
                ...
                # Extract parameters
                tool_params = {}
                param_matches = re.findall(r"<parameter name=\"([^\"]+)\">(.*?)</parameter>", invoke_content, re.DOTALL)
                ...
                for param_name, param_value in param_matches:
                    ...
                    if (param_value.startswith('{') and param_value.endswith('}')) or \
                       (param_value.startswith('[') and param_value.endswith(']')):
                        try:
                            tool_params[param_name] = json.loads(param_value)
                            continue
                        except json.JSONDecodeError:
                            pass
                    
                    # Otherwise convert to appropriate type
                    if param_value.lower() in ["true", "false"]:
                        tool_params[param_name] = param_value.lower() == "true"
                    elif param_value.isdigit():
                        tool_params[param_name] = int(param_value)
        ...
        if json_match:
            try:
                ...
                if (
                    ...
                    and ("tool" in json_data or "name" in json_data)
                    and ("parameters" in json_data or "arguments" in json_data)
                ):
                    tool_name = json_data.get("tool") or json_data.get("name", "")
                    tool_params = json_data.get("parameters") or json_data.get("arguments", {})
                    return tool_name, tool_params
            except json.JSONDecodeError:
                pass
    ...
    def format_system_prompt(self, agent: AgentProtocol, base_prompt: str, **kwargs: Any) -> str:
        """
        Format the system prompt for ReAct reasoning with enhanced tool integration.
        ...
        """
        # Get tools description from multiple possible sources
        tools_description = kwargs.get("tools_description", "")

        # Try to get from llm_provider for dynamic provider-specific formatting
        llm_provider = kwargs.get("llm_provider")
        provider_name = None
        if llm_provider:
            if hasattr(llm_provider, "name"):
                provider_name = llm_provider.name
            elif hasattr(llm_provider, "config") and "provider" in llm_provider.config:
                provider_name = llm_provider.config["provider"]
        
        # Check if the agent has tool descriptions as a parameter
        ...
        if not tools_description and hasattr(agent, "_tool_manager"):
            ...
            tool_manager = getattr(agent, "_tool_manager")
            if hasattr(tool_manager, "get_formatted_tool_descriptions"):
                # Pass provider name to potentially format tool descriptions appropriately
                if hasattr(tool_manager, "get_formatted_tool_descriptions") and "provider_name" in inspect.signature(tool_manager.get_formatted_tool_descriptions).parameters:
                    tools_description = tool_manager.get_formatted_tool_descriptions(
        ...
        # If we have a provider, try to get provider-specific prompt
        if provider_name:
            try:
                prompt_id = f"system.react.{provider_name.lower()}"
                provider_prompt = format_prompt(prompt_id, tools_description=tools_description)
                if provider_prompt:
                    logger.info(f"Using provider-specific prompt for {provider_name}")
                ...
                logger.debug(f"Error loading provider-specific prompt: {e}")
                # Continue to standard prompt
