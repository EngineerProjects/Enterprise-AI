...
def _register_builtin_frameworks() -> None:
    """Register built-in reasoning frameworks."""
    try:
        # Import and register the enhanced ReAct framework
        from enterprise_ai.agent.reasoning.enhanced_react import EnhancedReActReasoning
        register_framework("enhanced_react", EnhancedReActReasoning())
    except ImportError:
        logger.debug("Enhanced ReAct framework not available")
    
    try:
        # Import and register other frameworks as they become available
        from enterprise_ai.agent.reasoning.react import ReActReasoning
        register_framework("react", ReActReasoning())
    except ImportError:
        logger.debug("ReAct framework not available")
    
    try:
        from enterprise_ai.agent.reasoning.cot import ChainOfThoughtReasoning
        register_framework("cot", ChainOfThoughtReasoning())
    except ImportError:
        logger.debug("CoT framework not available")
    ...
    try:
        ...
        logger.debug("SWE framework not available")
    
    try:
        from enterprise_ai.agent.reasoning.mcp import MCPReasoning
        register_framework("mcp", MCPReasoning())
    except ImportError:
        logger.debug("MCP framework not available")


# Initialize built-in frameworks on module load
