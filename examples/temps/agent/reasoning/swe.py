...
class SoftwareEngineeringReasoning(ToolBasedReasoning):
    ...
    def _execute_dev_tool_with_handling(
        ...
        """
        Execute a development tool with enhanced error handling and logging.
        ...
        """
        logger.info(f"SWE Framework executing tool '{tool_name}' with params: {list(params.keys())}")
        
        if not hasattr(agent, "_agent_tools_manager"):
            error_msg = "Agent does not have a tool manager"
        ...
        if hasattr(agent, "ensure_tools_initialized"):
            agent.ensure_tools_initialized()
        ...
        if hasattr(tool_manager, "get_tool") and kwargs.get("validate_params", True):
            ...
            if tool and hasattr(tool, "parameters"):
                ...
                if not valid_params:
                    logger.warning(f"Invalid parameters for tool '{tool_name}': {error_msg}")
                    return ToolFailure(
        ...
        timeout = kwargs.get("tool_timeout", 30.0)  # Increased default timeout
        if tool_name.lower() in ["execute_code", "python_execute", "execute_python", "run_code"]:
            # Increase timeout for code execution
            timeout = kwargs.get("code_execution_timeout", 60.0)
        ...
        try:
            ...
            logger.info(f"Tool '{tool_name}' execution in SWE framework - Success: {success}")
            
            if success and result.output:
                # Limit output logging to prevent spam
                output_str = str(result.output)
                if len(output_str) > 500:
                    logger.debug(f"Tool '{tool_name}' output (truncated): {output_str[:500]}...")
                else:
                    logger.debug(f"Tool '{tool_name}' output: {output_str}")
            elif result.error:
