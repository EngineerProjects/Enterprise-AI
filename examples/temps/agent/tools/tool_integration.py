...
def parse_message_for_tool_calls(message: MessageProtocol, provider_name: Optional[str] = None) -> List[Dict[str, Any]]:
    """
    Parse a message for tool calls using the appropriate adapter.

    Args:
        message: Message to parse
        provider_name: Optional provider name to use specific adapter
    ...
    try:
        # First, try to get tool calls from metadata (most reliable)
        if hasattr(message, "metadata") and message.metadata:
            metadata = message.metadata
            if "tool_calls" in metadata:
                return metadata["tool_calls"]
        
        # Next, try to use the LLM adapter system if available
        try:
            from enterprise_ai.llm.adapters import create_adapter_for_provider
            
            # Determine provider from message metadata if not specified
            if not provider_name and hasattr(message, "metadata") and message.metadata:
                if "provider" in message.metadata:
                    provider_name = message.metadata["provider"]
            
            # Create appropriate adapter
            adapter = create_adapter_for_provider(provider_name or "generic")
            
            # Use adapter to extract tool calls
            tool_calls = adapter.extract_tool_calls(message)
            if tool_calls:
                return tool_calls
        except ImportError:
            # Fall back to legacy parser if adapters not available
            logger.debug("LLM adapters not available, falling back to legacy parser")
...
def format_tool_response_message(
    ...
    """
    Format a tool execution result as a user message, using adapters when available.

    Args:
        tool_name: Name of the executed tool
        tool_result: Result of tool execution
        tool_call_id: Optional ID of the tool call
        response_format: Format of the response ("json", "markdown", "react", or "provider")
        provider_name: Optional provider name to use a specific adapter format
    ...
    # If response format is "provider", try to use the adapter system
    if response_format == "provider" and provider_name:
        try:
            from enterprise_ai.llm.adapters import create_adapter_for_provider
            adapter = create_adapter_for_provider(provider_name)

            # Format the result using the provider's format (convert to dict for the adapter)
            formatted_response = FunctionCallingFormatter.format_tool_response(
                tool_name, tool_result, tool_call_id, include_metadata=True
            )

            # Create the message content based on provider
            if provider_name.lower() in ["anthropic", "claude"]:
                response_text = f"<tool_result>\n{json.dumps(formatted_response, indent=2)}\n</tool_result>"
            elif provider_name.lower() in ["openai", "azure"]:
                response_text = json.dumps(formatted_response)
            else:
                # Default format
                response_text = f"Tool result for {tool_name}: {str(tool_result.output)}"

            # Create message using appropriate role
            if tool_call_id:
                return cast(
                    MessageProtocol,
            ...
            # Fall back to default formatting if adapters not available
            logger.debug("LLM adapters not available, using default formatting")
            response_format = "json"  # Default to JSON format

    # Use standard formatting methods if not using provider-specific format
